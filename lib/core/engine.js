    ...
    try {
        var componentsData = keys.map(function (em, i) {
            try {
                ...
                throw( _.template('第<%=index%>个模块(编号<%=aid%>)存在错误:<%=message%>')({index:i,aid:aids[i],message: e.message}));
            }
            return o;
        });
        //按顺序合并上下边距
        ...
        try {
            _.each(keys, function (v, index) {
                try {
                    ...
                    throw(_('第<%=index%>个模块存在错误:<%=message%>')({index: index, message: e.message}));
                }
            });
            ...
            throw(_('生成版本<%=version%>的文件存在错误:<br/><%=message%>')({version:version,message: e.message}));
        }
    }
